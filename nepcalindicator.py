...
def contents_func():
    ...
    return " %s %s %s, %s" %(eng2nepnum(y), nepmonths[m], eng2nepnum(d), nd)
...
class NepaliDateIndicator:
    ...
    def menu_setup(self):
        self.menu = Gtk.Menu()
        self.nepdateconverter = Gtk.MenuItem("Convert Dates")
        self.nepdateconverter.connect("activate", self.nepdate)
        self.nepdateconverter.show()
        self.quit_item = Gtk.MenuItem("Quit")
        self.quit_item.connect("activate", self.quit)
        self.quit_item.show()

    def main(self):
        self.indicator.set_label(contents_func(), "")
        self.menu.append(self.nepdateconverter)
    ...
    def nepdate(self, widget):
        try:
            subprocess.call("nepdate")
        except:
            dialog = Gtk.MessageDialog()
            dialog.set_markup('''You must install nepdate found here:
            <span foreground="blue">https://launchpad.net/~shriteshb/+archive/nepdate</span>''')
            dialog.show()
            pass

if __name__ == "__main__":
    indicator = NepaliDateIndicator()
    try:
        indicator.main()
    except:
        pass
